...
async function buildTreeFast(rootUsername: string): Promise<UserNode | null> {
  ...
  function attachChildren(node: UserNode): number {
    ...
    let angka = node.children.length; // mulai dari total anak langsung
 console.log(angka)
    for (const child of node.children) {

      angka--; // turunkan
      attachChildren(child); // tetap proses anaknya
    }

    node.downlineCount = angka; // sisa angka setelah hitung mundur
    return angka;
  }
